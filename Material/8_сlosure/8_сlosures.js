...
Это важно понимать, что именно по мере выполнения. Именно поэтому очень важно объявлять переменные, потому что
...
Если в какой-то функции или блока кода , то создает эту запись в Environment у этой функции/блока кода.
...
Простыми словами LexicalEnviroment удаляется дворником, когда либо он становится не доступным либо 
пока есть кто-то или что, что ссылается на него (переменная или внутреняя функция) 
*/
function Timer() {
    let _time = 0;
